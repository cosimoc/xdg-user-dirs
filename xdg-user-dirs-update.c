...
save_locale (void)
{
  ...
  if (!g_file_set_contents (user_locale_file, locale, -1, &error))
    {
      g_printerr ("Can't save user-dirs.locale: %s\n", error->message);
      g_error_free (error);
    }
...
save_user_dirs (const char *dummy_file)
{
  ...
  fprintf (file, "# Format for top-level directories is XDG_xxx_DIR=\"$HOME/yyy\", where yyy is a shell-escaped\n");
